...
reg_dfw <- read_csv("../data/reg.csv") %>% 
  clean_names() %>% 
  mutate_at(.cols = "rep", .funs = factor)
...
```{r reg exp boxplot}
ggplot(data = reg_dfw, aes(x = sr_ksha, y = yield_mgha, fill = rep))+
  geom_boxplot()
```
...
```{r nd}
nd <- data.frame(sr_ksha = seq(min(reg_dfw$sr_ksha), max(reg_dfw$sr_ksha), 1))
...
ggplot(reg_dfw, 
       aes(x = sr_ksha))+
  geom_point(aes(y = yield_mgha),size = 3, 
             alpha = .7)+
  geom_line(data = nd, 
            aes(y = mod1_yield_mgha), 
            color = "forestgreen")+
  geom_ribbon(data = nd, aes(ymin = mod1_int_lcl, ymax = mod1_int_upl), alpha = 0.5, fill = "gray")
```
...
# Model fitting
mod2_lin <- lmer(data = reg_dfw,
                 yield_mgha ~ sr_ksha + (1|rep))
...
# Model fitting
mod3_quad <- lmer(data = reg_dfw, formula = yield_mgha ~ sr_ksha + I(sr_ksha^2) + (1|rep))
